...
template <class T>
Vector<T>::Vector() : arr(NULL), allocSize(0), size(0) {
}

template <class T>
Vector<T>::Vector(int amount) : arr(new T[amount]), allocSize(amount), size(0) {
}
...
template <class T>
void Vector<T>::remove(const T& rhs) {
	if (!isIn(rhs)) {
...
template <class T>
void Vector<T>::removeWithNoShuffle(int index) {
	if (index >= size || index < 0) {
		cerr << "Invalid index at Vector::removeWithNoShuffle(int).";;
		return;
	}
	if (size--) {
...
template <class T>
int Vector<T>::getAllocatedSize() const {
	return allocSize;
}
...
template <class T>
void Vector<T>::reallocate(int newAllocSize) {
...
template <class T>
void Vector<T>::copyFrom(const Vector<T>& rhs) {
...
template <class T>
bool Vector<T>::isIn(const T& rhs) const {
	for (int i = 0; i < size; ++i) {
		if (arr[i] == rhs) {
			return true;
		}
...
template <class T>
int Vector<T>::indexOf(const T& rhs) const {
	if (!isIn(rhs)) {
		return -1;
	}
	int index;
